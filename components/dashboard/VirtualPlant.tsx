...
export default function VirtualPlant({ stage = 1, health = 100, freezes = 0 }: VirtualPlantProps) {
  ...
  return (
    <motion.div 
      ...
      className="relative rounded-3xl p-6 border border-white/5 bg-[#0a0f1c]/80 backdrop-blur-md shadow-xl overflow-hidden group"
    >
      ...
      <div className="relative z-10 flex flex-col sm:flex-row items-center gap-6">
        ...
        {/* Plant Display */}
        <div className="relative w-32 h-32 flex items-center justify-center bg-black/40 rounded-full border border-white/10 shadow-inner overflow-hidden">
          {/* Ground */}
          <div className="absolute bottom-0 left-0 right-0 h-8 bg-[#1a1311] border-t border-white/5" />
          ...
          <motion.div
            ...
            className={`text-6xl z-10 ${health < 30 ? 'opacity-70 grayscale-[0.5]' : ''}`}
          ...
          {stage === 4 && health > 80 && (
             <Sparkles className="absolute top-4 right-4 text-pink-400 opacity-50 animate-pulse" size={16} />
          )}
        </div>
        ...
        <div className="flex-1 w-full text-center sm:text-left">
          <div className="flex items-center justify-center sm:justify-between mb-2">
            ...
            {freezes > 0 && (
              <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full text-blue-400 text-xs font-bold" title={`${freezes} Streak Freezes Active`}>
                <ShieldAlert size={12} />
                {freezes} Active
              </div>
            )}
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs font-medium">
              <span className="flex items-center gap-1 text-gray-400"><Droplet size={12}/> Moisture</span>
